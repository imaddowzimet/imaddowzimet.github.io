...
but I wasn't interested in those, and it seemed like it would be possible to extract the nationalities. What's more, there were only 4,658 pieces with missing data, which isn't bad out of 130,409 pieces overall (4%).  
...
Hmm. I started out with an N of 130,409, of which 4,658 had missing data for the artists bio field. 130,409 - 4,658 = 125,751, but then I know some pieces are double counted, so I should have **more** than that number. Instead, I have less. Probably what happened is that some of the artist's bio fields had information but not valid nationalities, so they weren't initially captured in my missing data check; since I don't have a good way to impute these though, and the drop in size doesn't seem alarming, I proceed onwards and upwards.  
