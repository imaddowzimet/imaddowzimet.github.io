...
So we may need to forget about trying to look at absolute levels, and instead just focus in on relative shifts - on other words, just look at distributions over time. I'm actually not sure what I did next works out mathematically (I'm actually fairly sure it doesn't), but the result was so pretty that I thought it was worth inclduing here. What I did was take the logged values for each 5-year period, and standardized them so that the sum of the logs always add up to the same number (10 in this case); what I ended up with was this piece of art:
